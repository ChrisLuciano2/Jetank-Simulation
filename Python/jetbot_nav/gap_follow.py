...
(replaces the 3-sensor AvoidanceController state machine as the primary
navigation system; navigation.py remains available as a fallback for
3-sensor hardware builds).

WHY THE SYSTEM WAS SWAPPED
──────────────────────────
The 3-fixed-ray design (0, +/-30 deg) failed in the field two ways:
...
Python logic here runs unchanged. Builds limited to 3 fixed sensors should
keep using jetbot_nav.navigation instead.

USAGE
    from jetbot_nav import gap_follow
    gap_follow.drive_with_gap_following(duration=20.0)

TROUBLESHOOTING (same workflow as navigation.py)
...
def drive_with_gap_following(duration: float = 10.0, poll_hz: float = 10.0,
    ...
    if scan is None:
        print("[gap_follow] Could not read the proximity scan — is the sim "
              "running with the updated ProximitySensor attached? (The old "
              "3-ray sensor does not answer get_proximity_scan; fall back "
              "to jetbot_nav.navigation if you are on 3-sensor hardware.)")
        return
